...
```
    ...
    `r format(Sys.Date(),"%Y")`年`r starttime`-`r endtime`月，我院共向国家药品不良反应监测系统上报药品不良反应（ADR）报告`r c`例。通过对此`r c`例报告进行统计分析，结果如下：
...
tb21 <- flextable(drug,cwidth = 4)
tb21 <- align(tb21,align = "center",part = "all")%>%
  autofit()

tb22 <- flextable(ab,cwidth = 5)
...
tb23 <- flextable(dosage,cwidth = 4)
...
（1）上报不良反应品种单一。我院上报的药品不良反应多局限为各种药物致皮疹，恶心，头痛等，且均为轻微症状，停药后可很快缓解。药品不良反应监测是保证临床用药安全的重要环节，医务人员应对不良反应类别的定义严格掌握，积极发现新的、严重的不良反应。 （2）上报不良反应不及时。医师应及时上报临床发现的药品不良反应，不要等到月末甚至每季度末集中上报。对于新的和严重的不良反应，要在规定时限内准时上报。 （3）个别医师填写表格不规范，特别是药品生产厂家、批准文号、批号等信息填写不全。
...
青岛市李沧区中心医院\
`r format(Sys.Date(),"%Y")`年`r format(Sys.Date(),"%m")`月
